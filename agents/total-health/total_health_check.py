...
def section_13_governance_pipeline(report: HealthReport) -> None:
    ...
    if os.path.isdir(proposed_dir):
        ...
        # 13c-2: Proposal staleness — surface proposals stuck in voting/open >14 days
        stale_proposals = []
        ...
        for fname in os.listdir(proposed_dir):
            ...
            try:
                ...
                pass
        if stale_proposals:
            report.add(Check(13, f"Stale proposals (>{staleness_threshold.days}d in voting/open)",
                              False,
                              f"{len(stale_proposals)} stale: {'; '.join(stale_proposals[:5])}",
                              "warning"))
        else:
            report.add(Check(13, "No stale proposals", True))
    else:
