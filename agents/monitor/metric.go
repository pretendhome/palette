...
// Monitor never interprets these — it emits them and routes them.
...
// locally or over SSH, identical to the Debugger's ProbeFunc pattern.
type MetricFunc func(run MetricRunner, cfg Config) MetricSignal

// metricChain is the ordered list of all metrics Monitor tracks.
...
func checkTunnelPort(_ MetricRunner, cfg Config) MetricSignal {
	...
	return sigalert("TunnelPort",
		...
		baseline,
		fmt.Sprintf("SSH tunnel not listening on port %d — run: debugger tunnel --remote %s",
...
func checkBridgePort(run MetricRunner, cfg Config) MetricSignal {
	...
	return sigalert("BridgePort",
		...
		baseline,
		fmt.Sprintf("socat bridge not running at %s:%d — run: debugger fix --remote %s",
...
func checkUpstreamRoute(_ MetricRunner, cfg Config) MetricSignal {
	...
	if source == "local_fallback" || source == "" {
		return sigalert("UpstreamRoute",
			...
			"source=openclaw_*",
			"adapter is not reaching upstream — run: debugger diagnose")
	}
