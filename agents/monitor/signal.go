...
// Monitor's core job: watch everything in parallel, emit signals independently.
...
// Monitor never decides what to do with the signal — it just packages and routes.
func toPacket(sig MetricSignal, cfg Config) core.HandoffPacket {
	return core.HandoffPacket{
		ID:      newUUID(),
		TraceID: cfg.TraceID,
		From:    core.AgentMonitor,
		To:      sig.RouteTo,
