#!/usr/bin/env python3
"""
resolver.py — Resolver (Intent Resolver)
...
def refine_prompt(
    ...
    prompt = (
        ...
        f"  researcher  → shallow research, investigation, or information gathering\n"
        f"  perplexity.computer → deep research, web interaction, or competitive intelligence\n"
        f"  debugger    → debugging, troubleshooting, or diagnosing failures\n"
        f"  monitor     → monitoring, alerting, or signal watching\n"
        f"  narrator    → narrative, pitch, presentation, or communication\n\n"
        f"Return JSON only:\n"
        f'{{ "refined_task": "...", '
...
def main() -> int:
    ...
    progress(f"resolver v0.1 | turn {turn}/{max_turns}")
