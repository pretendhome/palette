...
if [ "$SKIP_KEYS" = "--skip-keys" ]; then
  ...
  # Also collect optional Mistral key
  read -rp "  Mistral API key (for critique/create — optional): " MISTRAL_KEY_INPUT
  if [ -n "$MISTRAL_KEY_INPUT" ]; then
    KEYS="${KEYS}MISTRAL_API_KEY=$MISTRAL_KEY_INPUT\n"
    ok "Mistral key set"
  else
    info "Skipped — get free key at console.mistral.ai"
  fi
  ...
  # Generate LiteLLM config from collected keys (if litellm is installed)
  if command -v litellm &>/dev/null; then
    LITELLM_CFG="$SCRIPT_DIR/litellm_config.yaml"
    cat > "$LITELLM_CFG" << 'LITELLM_HEADER'
# LiteLLM Config — Mission Canvas Model Router
# Auto-generated by setup.sh. Only providers with keys are included.
# Start manually: litellm --config litellm_config.yaml --port 4000

model_list:
  # Local (Ollama — always available, no key needed)
  - model_name: local
    litellm_params:
...
EOF
    ...
    cat >> "$LITELLM_CFG" << 'LITELLM_FOOTER'

general_settings:
  master_key: sk-mc-local
LITELLM_FOOTER

    ok "LiteLLM config generated ($(grep -c model_name "$LITELLM_CFG") models)"
  fi
...
# Start LiteLLM router (if installed and config exists)
if command -v litellm &>/dev/null && [ -f "$SCRIPT_DIR/litellm_config.yaml" ]; then
  pkill -f "litellm.*config" 2>/dev/null || true
  sleep 0.3
  (cd "$SCRIPT_DIR" && litellm --config litellm_config.yaml --port 4000 > /tmp/mc-litellm.log 2>&1 &)
  sleep 2
  if curl -s http://127.0.0.1:4000/health >/dev/null 2>&1; then
    ok "LiteLLM router (port 4000) — all models via one proxy"
  else
    warn "LiteLLM may be starting — check /tmp/mc-litellm.log"
  fi
else
  info "LiteLLM not found — using direct provider connections"
  info "Install: pip install litellm (optional, enables unified model routing)"
fi
...
if curl -s http://127.0.0.1:4000/health >/dev/null 2>&1; then
echo -e "  ${DIM}LiteLLM:${NC}      http://localhost:4000 (unified model router)"
fi
